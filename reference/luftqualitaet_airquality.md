...
A tibble with one row per air-quality measurement record. Guaranteed
columns follow the API `indices` metadata; rows contain indexed scalar
values with names resolved from `indices` when provided.
