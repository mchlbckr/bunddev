...
A tibble with one row per charging-station entry. Guaranteed columns
are: `road_id`, `identifier`, `title`, `subtitle`, `display_type`,
`icon`, `is_blocked`, `future`, `start_timestamp`, `start_time`,
`point`, `extent`, `coordinate_lat`, `coordinate_long`, `description`,
`footer`, `route_recommendation` (list-column), and
