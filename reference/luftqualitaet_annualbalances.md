...
row per annual-balance record. Column names are derived from the
`indices` metadata in the API response and typically include
`station id`, `component id`, `year`, `value`, and `transgression type`.

## Details

Returns annual balance data for a component and year. API documentation:
