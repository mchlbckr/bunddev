...
This vignette demonstrates a typical workflow using the **bunddev**
package to discover an API, inspect its parameters, set up
authentication (if needed), retrieve tidy data, and visualise it.
...
You can store your API keys securely in a `.Renviron` file, which R
loads automatically at startup. Add a line like the following to
`~/.Renviron` (or a project‑specific `.Renviron` in the package root):
...
with [`Sys.setenv()`](https://rdrr.io/r/base/Sys.setenv.html) as shown
earlier.
...
*This vignette is intentionally simple and runs without contacting any
live API (except when you actually execute the examples). It is designed
to work during package checks and as a quick reference for users.*
